...
public class NovaAPI implements IaasApi, IaaSMonitoringApi {
    ...
    public static IaasApi getNovaAPI(Map<String, String> args)
        ...
        return getNovaAPI(args.get(NovaAPIConstants.ApiParameters.USER_NAME),
                ...
                args.get(NovaAPIConstants.ApiParameters.TENANT_NAME), new URI(
                        args.get(NovaAPIConstants.ApiParameters.API_URL))
        /* null */);
    }

    public static synchronized NovaAPI getNovaAPI(String username,
            String password, String tenantName, URI endpoint)
        ...
        if (instance == null || !isValid(instance.created)) {
            try {
                ...
                throw new AuthenticationException("Failed to authenticate to "
                        + endpoint, t);
            }
    ...
    private NovaAPI(String username, String password, String tenantName,
            URI endpoint) throws IOException {
        logger.debug("Initializing Nova API for Nova infrastructure...");
        logger.debug(String.format("username='%s', tenant='%s', endpoint='%s'",
    ...
    private void authenticate(String username, String password, String tenant)
            throws IOException {
        ...
        jsonAuth.put(NovaAPIConstants.ApiParameters.PASSWORD_CREDENTIALS,
                jsonCreds);
        ...
        try {
            ...
            throw new RuntimeException(
                    "Cannot parse service catalog - check your tenant name");
        }
    }

    public void listAvailableImages() throws ClientProtocolException,
            IOException {
    ...
    public String createServer(String name, String imageRef, String flavorRef,
    ...
    public boolean rebootServer(String serverId, String method)
            throws ClientProtocolException, IOException {
    ...
    public String filter(String ressources, String criteria)
            throws ClientProtocolException, IOException {
    ...
    private HttpResponse post(String path, JSONObject content)
            throws ClientProtocolException, IOException {
    ...
    private HttpResponse get(String path) throws ClientProtocolException,
            IOException {
    ...
    private HttpResponse delete(String path) throws ClientProtocolException,
            IOException {
    ...
    public IaasInstance startInstance(Map<String, String> arguments)
        ...
        String userData = "";
        if (arguments
                .containsKey(NovaAPIConstants.InstanceParameters.USER_DATA)) {
            userData = arguments
                    .get(NovaAPIConstants.InstanceParameters.USER_DATA);
            userData = new String(Base64.encodeBase64(userData.getBytes()));
        }

        return new IaasInstance(createServer(
                arguments.get(NovaAPIConstants.InstanceParameters.NAME),
                arguments.get(NovaAPIConstants.InstanceParameters.IMAGE_REF),
                arguments.get(NovaAPIConstants.InstanceParameters.FLAVOR_REF),
                userData, metaData));
    }
    ...
    @Override
    public String[] getHosts() throws ClientProtocolException, IOException {
        HttpResponse response = get("/os-hosts");
        String entity = EntityUtils.toString(response.getEntity());
        JSONArray array = JsonPath.read(entity,
                "$.hosts[?(@.service=='compute')].host_name");
        String[] stringArray = Arrays.copyOf(array.toArray(), array.size(),
                String[].class);
        return stringArray;
    }
    ...
    public Map<String, String> getVMProperties(String vmId) throws Exception {
        ...
        String entity = EntityUtils.toString(response.getEntity());
        JSONObject resource = (JSONObject) ((List<Object>) JsonPath.read(
                entity, "$.servers[*]")).get(0);
        Map<String, String> properties = new HashMap<String, String>();
        properties.put("status", "" + resource.get("status"));
        return properties;
    }

    private void addProperties(String entity, String filterToken,
            Map<String, String> properties) {
        JSONObject resource = (JSONObject) ((List<Object>) JsonPath.read(
                entity, host_prop_filter_criteria.replace("$FILTER_TOKEN$",
                        filterToken))).get(0);
